...
func registerPipelineRoutes(r *gin.Engine, auth, m2mAuth gin.HandlerFunc, deps PipelineRouteDeps) {
	...
	).WithStore(deps.SQLiteStore).WithTaskReader(deps.TaskReader).WithAssetService(deps.AssetService).WithStockFolderLister(deps.StockFolderLister).WithIntakeSink(velmetrics.NewCreatorIntakeSink()).WithIntakeSourceRecorder(velmetrics.NewIntakeSourceSink()).RegisterRoutes(r, auth, m2mAuth, pipeline.NewM2MOrAdminAuthMiddleware(deps.Cfg, deps.SQLiteStore, nil, deps.Cfg.Auth.AdminToken, auth))
}
