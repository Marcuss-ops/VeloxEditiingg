...
func registerPipelineRoutes(r *gin.Engine, auth, m2mAuth gin.HandlerFunc, deps PipelineRouteDeps) {
	...
	).WithStore(deps.SQLiteStore).WithTaskReader(deps.TaskReader).WithAssetService(deps.AssetService).WithIntakeSink(velmetrics.NewCreatorIntakeSink()).WithLegacyBodySink(velmetrics.NewLegacyBodySink()).RegisterRoutes(r, auth, m2mAuth)
}
