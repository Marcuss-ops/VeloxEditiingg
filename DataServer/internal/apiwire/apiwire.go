...
type SubmitJobRequest struct {
	...
	// RuntimeAssets and RuntimePayload make PRE single-stage when the producer
	// already knows the runtime assets at submit time. A nil RuntimeAssets
	// keeps the legacy two-stage PRE→FINALIZE contract; an explicitly supplied
	// array is only considered complete when it is non-empty or accompanied by
	// RuntimeAssetsComplete=true.
