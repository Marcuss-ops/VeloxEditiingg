...
// idempotently. Splits runID to recover the workerID; the
// "smoke-<workerID>-<nanos>" convention is enforced by the
// executor's runID assignment.
func (r *RegistryDrainLease) ReleaseSmokeLease(_ context.Context, runID string) error {
	if r == nil || r.Reg == nil {
		return nil
	}
	parts := strings.SplitN(runID, "-", 3)
	if len(parts) >= 3 && parts[0] == "smoke" {
		if err := r.Reg.SetWorkerDrain(context.Background(), parts[1], false); err != nil {
