...
//
// PR15.5: LifecycleService now uses jobs.Repository (canonical domain
...
type LifecycleService struct {
	jobsRepo jobs.Repository // canonical domain surface (PR15.5: sole write surface)
	clock    clock.Clock
}
...
func NewLifecycleService(jobsRepo jobs.Repository, c clock.Clock) (*LifecycleService, error) {
	if jobsRepo == nil {
...
func (l *LifecycleService) RequeueExpiredLeases(ctx context.Context, limit int) ([]jobs.RequeueResult, error) {
	if limit <= 0 {
		limit = 100
	}
...
func (l *LifecycleService) GetJobsByStatus(ctx context.Context, status JobStatus) ([]*Job, error) {
	domainJobs, err := l.jobsRepo.List(ctx, jobs.Filter{
		Statuses: []jobs.Status{jobs.Status(status)},
		Limit:    1000,
	})
	if err != nil {
	...
	for _, j := range domainJobs {
		// Build a minimal queue.Job from the canonical jobs.Job.
		job := &Job{
			JobID:       j.ID,
...
func (l *LifecycleService) GetNextJobID(ctx context.Context) (string, error) {
	pending, err := l.jobsRepo.List(ctx, jobs.Filter{
		Statuses: []jobs.Status{jobs.StatusPending},
		Limit:    1,
	})
	if err != nil {
		return "", err
