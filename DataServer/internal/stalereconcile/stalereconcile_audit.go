...
// auditEventForFinding builds the audittrail.Event for a single finding. The
// event ID is deterministic over the finding identity so a replay never
// duplicates an audit row.
...
// finding inside an open transaction. Leaf-private copy of the store helper so
// this package stays free of internal/store.
