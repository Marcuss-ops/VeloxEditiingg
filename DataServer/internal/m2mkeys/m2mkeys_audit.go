// Package m2mkeys / m2mkeys_audit.go
//
...
// the response.
func AppendM2MAuditLog(ctx context.Context, db *sql.DB, e M2MAuditEntry) error {
	if e.Method == "" {
		e.Method = "POST"
	}
	if e.Path == "" {
	...
	if err != nil {
		return fmt.Errorf("m2mkeys: AppendM2MAuditLog: %w", err)
	}
	return nil
}
...
func ListM2MAuditLog(ctx context.Context, db *sql.DB, clientID string, limit int) ([]M2MAuditEntry, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("m2mkeys: ListM2MAuditLog: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&id, &clientID, &idemHash, &method, &path, &status, &scope,
			...
			return nil, fmt.Errorf("m2mkeys: ListM2MAuditLog scan: %w", err)
		}
		createdT, err := parseSQLiteTime(createdAt)
		if err != nil {
			return nil, fmt.Errorf("m2mkeys: ListM2MAuditLog created_at: %w", err)
		}
