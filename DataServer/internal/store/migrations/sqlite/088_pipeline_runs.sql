...
--
-- Idempotency: CREATE TABLE IF NOT EXISTS / CREATE INDEX IF NOT EXISTS are
-- safe for re-runs. The migration runner's checksum gate prevents modification
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_pipeline_runs_idempotency_key
...
CREATE INDEX IF NOT EXISTS idx_pipeline_runs_remote_job_id
    ON pipeline_runs(remote_job_id)
    WHERE remote_job_id IS NOT NULL AND remote_job_id != '';

CREATE INDEX IF NOT EXISTS idx_pipeline_runs_velox_job_id
