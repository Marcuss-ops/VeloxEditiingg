...
func TestRecordCreatorForwardingPoll_FencedSuccess(t *testing.T) {
	...
	leases, err := db.ClaimCreatorForwardings(ctx, "runner-poll", "cf", 5*time.Minute, 1)
	if err != nil || len(leases) != 1 {
		t.Fatalf("claim: err=%v len=%d", err, len(leases))
	}
	lease := leases[0]
	nextPoll := time.Now().UTC().Add(time.Minute)
	if err := db.RecordCreatorForwardingPoll(ctx, lease.ForwardingID, lease.RunnerID, lease.LeaseID, "running", nextPoll); err != nil {
...
func TestRecordCreatorForwardingPoll_RejectsWrongRunner(t *testing.T) {
	...
	leases, err := db.ClaimCreatorForwardings(ctx, "runner-owner", "cf", 5*time.Minute, 1)
	if err != nil || len(leases) != 1 {
		t.Fatalf("claim: err=%v len=%d", err, len(leases))
	}
	lease := leases[0]
	if err := db.RecordCreatorForwardingPoll(ctx, lease.ForwardingID, "runner-other", lease.LeaseID, "running", time.Now().UTC()); !errors.Is(err, ErrLeaseLost) {
...
func TestRecordCreatorForwardingPoll_RejectsWrongLease(t *testing.T) {
	...
	lease := leases[0]
	if err := db.RecordCreatorForwardingPoll(ctx, lease.ForwardingID, lease.RunnerID, "lease-stale", "running", time.Now().UTC()); !errors.Is(err, ErrLeaseLost) {
...
func TestRecordCreatorForwardingPoll_RejectsExpiredLeaseAndPreservesNextPoll(t *testing.T) {
	...
	leases, err := db.ClaimCreatorForwardings(ctx, "runner-expired", "cf", -time.Second, 1)
	if err != nil || len(leases) != 1 {
		t.Fatalf("claim: err=%v len=%d", err, len(leases))
	}
	lease := leases[0]
	...
	if err := db.RecordCreatorForwardingPoll(ctx, lease.ForwardingID, lease.RunnerID, lease.LeaseID, "running", time.Now().UTC()); !errors.Is(err, ErrLeaseLost) {
...
func TestRecordCreatorForwardingPoll_TakeoverRejectsStaleLease(t *testing.T) {
	...
	oldLeases, err := db.ClaimCreatorForwardings(ctx, "runner-old", "cf", -time.Second, 1)
	if err != nil || len(oldLeases) != 1 {
		t.Fatalf("old claim: err=%v len=%d", err, len(oldLeases))
	}
	oldLease := oldLeases[0]
	newLeases, err := db.ClaimCreatorForwardings(ctx, "runner-new", "cf", 5*time.Minute, 1)
	if err != nil || len(newLeases) != 1 {
		t.Fatalf("takeover claim: err=%v len=%d", err, len(newLeases))
	}
	if err := db.RecordCreatorForwardingPoll(ctx, oldLease.ForwardingID, oldLease.RunnerID, oldLease.LeaseID, "stale", time.Now().UTC()); !errors.Is(err, ErrLeaseLost) {
		t.Fatalf("takeover error = %v, want ErrLeaseLost", err)
	}
	row, err := db.GetCreatorForwarding(ctx, oldLease.ForwardingID)
	if err != nil {
		t.Fatalf("get forwarding: %v", err)
	}
...
func assertPollUnchanged(t *testing.T, db *SQLiteStore, forwardingID string, wantAttempts int, wantStatus string) {
	t.Helper()
	row, err := db.GetCreatorForwarding(context.Background(), forwardingID)
	if err != nil {
		t.Fatalf("get forwarding: %v", err)
	}
