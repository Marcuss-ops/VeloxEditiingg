...
func projectPhaseAggregates(ctx context.Context, tx *sql.Tx, attemptID string) error {
	...
	_, err := tx.ExecContext(ctx, `
		...
		-- The concat actions are mutually exclusive phase representations of the
		-- same attempt-level work. Mixed packet telemetry can contain both the
		-- aggregate concat row and a packet mux row, so MAX avoids double-counting
		-- the same exclusive/per_attempt duration.
		engine_concat_ms = CASE WHEN engine_concat_ms > 0 THEN engine_concat_ms ELSE COALESCE((
			SELECT MAX(duration_ms) FROM task_phase_timings
