...
// store_smoke_runs.go owns the smoke_runs analytics table:
//
//   - smoke_runs records the END-TO-END duration baseline for
//     every Level D smoke executed by the LevelDSmokeExecutor
//     (fleet.LevelDSmokeExecutor — Step 12/15).
//   - Distinct from fleet_operations (audit trail) and
//     deployment_records (deploy lifecycle). The duration_ms
//     column is the operator's p-baseline for the dashboard's
//     "smoke runs slower than usual?" alerting.
//
// All terminal transitions are atomic UPDATE; the dashboard's
// "running smokes" view never observes a torn
// (status=SUCCEEDED, finished_at=NULL) row.
//
...
// CreateSmokeRunsTableIfNotExists is the test/dev-only
// bootstrap path. Idempotent; the DDL mirrors the migration
// file's CREATE TABLE exactly (modulo inline CHECK (duration_ms >= 0)
...
// CHECK constraint and confuse the dashboard's running-vs-
// completed view).
//
// started_at + finished_at are both stamped at insert via
// e.Now() so duration_ms can be derived mid-execution without
// re-querying. finished_at=DUMMY_TIME is a temporary sentinel
// replaced on terminal transition; final dashboards ignore
// non-terminal rows.
...
// Called once at the end of the LevelDSmokeExecutor's forward
// pipeline.
...
// Called from the LevelDSmokeExecutor's cleanup cascade when any
// phase fails.
...
// GetLatestSmokeForWorker returns the row with the highest
// started_at for the worker, regardless of status. Returns
// ErrSmokeRunNotFound when no rows exist.
...
// DESC order. limit <= 0 means no cap. Used by the operator
// dashboard's "smoke history" view.
...
// scanSmokeRun reads one row into a SmokeRun. Pulled out so
// the same SQL row → Go struct mapping happens in both Get /
// List without drift.
