...
// store_worker_validation.go owns the worker_validations table:
// schema bootstrap, single-row upsert, single-row read, list-all read.
// The table is independent from the worker snapshot — a worker can have
// zero or more validation runs over its lifetime, but the snapshot row
