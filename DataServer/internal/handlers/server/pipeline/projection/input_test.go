...
func TestBuildRawPayload_PreservesNestedRenderBoundary(t *testing.T) {
	raw := BuildRawPayload(SubmissionInput{
		...
		AudioTracks: []AudioTrackInput{{SourceURL: "velox-asset://music.mp3", Role: "background_music", Volume: 0.2}},
	})
	...
	tracks, ok := raw["audio_tracks"].([]interface{})
	if !ok || tracks[0].(map[string]interface{})["role"] != "background_music" {
		t.Fatalf("audio_tracks = %#v", raw["audio_tracks"])
	}
}
