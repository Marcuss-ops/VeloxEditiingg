...
func TestFinalizeVerified_BudgetConsumedBy429Retries(t *testing.T) {
	...
	runFinalize(t, db, resolver, artifacts.FinalizeVerifiedCommand{UploadID: "up-429", ArtifactID: "art-429", JobID: "J-429", WorkerID: "w", LeaseID: "l", AttemptNumber: 1, ExpectedRevision: 1, StorageProvider: "local", StorageKey: "artifacts/J-429/1", SHA256: testSHA256, SizeBytes: 1024, MIMEType: "video/mp4", VerifiedAt: time.Now().UTC()})
	var maxAttempts int
...
func TestFinalizeVerified_MarksTaskSucceeded(t *testing.T) {
	for _, status := range []string{"RUNNING", "LEASED", "PENDING"} {
		t.Run(status, func(t *testing.T) {
			...
			runFinalize(t, db, nil, artifacts.FinalizeVerifiedCommand{UploadID: "up-task", ArtifactID: "art-task", JobID: "J-task", WorkerID: "w", LeaseID: "l", AttemptNumber: 1, ExpectedRevision: 1, StorageProvider: "local", StorageKey: "artifacts/J-task/1", SHA256: testSHA256, SizeBytes: 1024, MIMEType: "video/mp4", VerifiedAt: time.Now().UTC()})
			var got string
...
func TestFinalizeVerified_ClosesAllStateTablesAtomically(t *testing.T) {
	...
	resolver := deliveries.NewSQLiteDeliveryPlanResolver(db)
	runFinalize(t, db, resolver, artifacts.FinalizeVerifiedCommand{UploadID: uploadID, ArtifactID: artifactID, JobID: jobID, WorkerID: "w", LeaseID: "l", AttemptNumber: 1, ExpectedRevision: 1, StorageProvider: "local", StorageKey: "artifacts/" + jobID + "/1", SHA256: testSHA256, SizeBytes: 1024, MIMEType: "video/mp4", VerifiedAt: time.Now().UTC()})
