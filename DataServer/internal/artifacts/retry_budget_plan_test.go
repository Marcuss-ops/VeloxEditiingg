...
func TestFinalizeVerified_StampsRetryBudgetFromPlan(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			for destID,expected:=range c.expected { var got int; if err:=db.QueryRow(`SELECT max_attempts FROM job_deliveries WHERE artifact_id = ? AND destination_id = ?`,"art-prop",destID).Scan(&got); err!=nil { t.Fatalf("query %s: %v",destID,err) }; if got!=expected { t.Errorf("%s: %s max_attempts=%d want %d",c.name,destID,got,expected) } }
		})
	}
}
...
func TestFinalizeVerified_FallbackMaxAttemptsWhenNoResolver(t *testing.T) {
	...
	rows,err:=db.Query(`SELECT destination_id, max_attempts FROM job_deliveries WHERE artifact_id = ? ORDER BY destination_id`,"art-fb"); if err!=nil { t.Fatal(err) }; defer rows.Close(); count:=0
	for rows.Next(){ var d string; var m int; if err:=rows.Scan(&d,&m); err!=nil { t.Fatal(err) }; count++; if m!=5 { t.Errorf("fallback %s max_attempts=%d want 5",d,m) } }
	if count!=2 { t.Fatalf("expected 2 fallback rows, got %d",count) }
}
...
func TestFinalizeVerified_SingleDestinationDefaultsToFive(t *testing.T) {
	...
	reader:=artifacts.NewSQLiteArtifactReader(db); fin:=artifacts.NewSQLiteFinalizeWriter(db,reader,nil)
	_,err:=fin.FinalizeVerified(context.Background(),artifacts.FinalizeVerifiedCommand{UploadID:"up-single",ArtifactID:"art-single",JobID:"J-single",WorkerID:"w",LeaseID:"l",AttemptNumber:1,ExpectedRevision:1,DestinationID:"primary",StorageProvider:"local",StorageKey:"artifacts/J-single/1",SHA256:"deadbeef",SizeBytes:1024,MIMEType:"video/mp4",VerifiedAt:time.Now().UTC()}); if err!=nil { t.Fatal(err) }
	var got int; if err:=db.QueryRow(`SELECT max_attempts FROM job_deliveries WHERE artifact_id=? AND destination_id='primary'`,"art-single").Scan(&got); err!=nil { t.Fatal(err) }; if got!=5 { t.Errorf("max_attempts=%d want 5",got) }
}
...
func TestFinalizeVerified_ZeroMaxAttemptsRevertsToDefault(t *testing.T) {
	...
	var got int; if err:=db.QueryRow(`SELECT max_attempts FROM job_deliveries WHERE artifact_id=? AND destination_id='primary'`,"art-zero").Scan(&got); err!=nil { t.Fatal(err) }; if got!=5 { t.Errorf("max_attempts=%d want 5",got) }
}
