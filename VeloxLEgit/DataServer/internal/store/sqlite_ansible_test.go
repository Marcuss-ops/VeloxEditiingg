...
// ============================================================
//
// PR3 fix: the legacy `ansible_computers` table was dropped by migration 008
// and the corresponding legacy CRUD methods on *SQLiteStore were removed.
// The original TestAnsibleComputersLegacy referenced them inline. Rather
// than reintroduce dead methods, the entire test block is replaced with this
// tombstone comment so future readers see WHY the scenarios can no longer
// be exercised. The canonical successor paths live in TestAnsibleHostsCRUD
// and TestAnsibleHostsListAndDelete (S11-migration success path).
//
// `_ = ansibleComputersLegacyTableDropped { bool }` keeps go vet quiet on
// zero-use declarations; the marker is documentation-only.
//
// (No test function: the legacy tests used `t.Skip()` AFTER referencing
// methods that no longer compile, so the entire body was retired rather
// than re-shaped.)
