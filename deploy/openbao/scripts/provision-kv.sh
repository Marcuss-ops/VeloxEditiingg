...
# ── TLS pinning for curl ─────────────────────────────────────────────────────
# The listener cert is self-signed (generated by gen-tls.sh into the state
# dir); pin it as the CA instead of -k so the API write still verifies the
# server identity. The `bao` CLI equivalent is BAO_SKIP_VERIFY=true.
curl_tls=(-k)
