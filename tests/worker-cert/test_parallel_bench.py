...
from bench import (
    CapResult, JobResult,
    parse_prometheus, extract_observations, percentile, normalize_ratio,
    choose_limit, classify_bottleneck, dynamic_cap_search,
    cap_matrix,
)
from bench.gates import (
    check_hard_stop_gates as _check_hard_stop_gates,
    hard_stops_passed as _hard_stops_passed,
    passes_safety_gates as _passes_safety_gates,
)
from bench.metrics import peak_memory_ratio as _peak_memory_ratio
from bench.runner import run_correctness_command
...
"""
        ...
        observations = extract_observations(values)
    ...
    def test_percentile_is_interpolated_p95(self) -> None:
        self.assertAlmostEqual(percentile([100, 200, 300, 400], 0.95), 385)

    def test_decision_uses_correct_videos_per_hour(self) -> None:
        def result(cap: int, correct_per_hour: float) -> CapResult:
            return CapResult(
                max_active_jobs=cap,
                status="PASS",
                wall_time_ms=1000,
        ...
        choose_limit(results, min_gain_pct=5, max_p95_ms=None, max_error_rate=0, max_iowait=0.35, max_peak_memory_ratio=0.85, max_fd_util_ratio=0.80, min_disk_free_bytes=10_000_000_000)
    ...
    def test_incomplete_without_correctness_hook_cannot_be_certified(self) -> None:
        job = JobResult("job-1", "SUCCEEDED", 100, correct=None, correctness_error="hook missing")
        self.assertIsNone(job.correct)

    def test_ratio_rejects_ambiguous_units(self) -> None:
        self.assertIsNone(normalize_ratio(2.0))
        self.assertAlmostEqual(normalize_ratio(1_000_000), 1.0)
    ...
    def test_correctness_hook_rejects_untrusted_artifact_urls(self) -> None:
        ...
        correct, detail = run_correctness_command(
    ...
    def test_peak_memory_ratio_rejects_high_usage(self) -> None:
        ...
        def result(cap: int, peak_ratio: float | None) -> CapResult:
            return CapResult(
                max_active_jobs=cap,
                status="PASS",
                wall_time_ms=1000,
    ...
    def test_peak_memory_ratio_085_threshold(self) -> None:
        ...
        choose_limit(results, min_gain_pct=5, max_p95_ms=None, max_error_rate=0,
    ...
    def test_compute_peak_memory_ratio(self) -> None:
        ...
        self.assertAlmostEqual(_peak_memory_ratio(3_000_000_000, 5_000_000_000), 0.375)
        self.assertIsNone(_peak_memory_ratio(None, 5_000_000_000))
        self.assertIsNone(_peak_memory_ratio(3_000_000_000, None))
        self.assertIsNone(_peak_memory_ratio(0, 0))

    def _make_cap_result(self, cap: int, correct_per_hour: float, **overrides: object) -> CapResult:
        ...
        return CapResult(**defaults)  # type: ignore[arg-type]
    ...
    def test_cap_matrix_measures_every_cap_through_configured_limit(self) -> None:
        self.assertEqual(cap_matrix(6), (1, 2, 3, 4, 5, 6))
    ...
    def test_bottleneck_classifier_uses_canonical_precedence(self) -> None:
        ...
        self.assertEqual(classify_bottleneck(base), "UNKNOWN")
        self.assertEqual(classify_bottleneck(
    ...
    def test_bottleneck_classifier_gpu_beats_cpu(self) -> None:
        ...
        self.assertEqual(classify_bottleneck(result), "GPU_BOUND")
    ...
    def test_bottleneck_classifier_gpu_unknown_does_not_classify(self) -> None:
        ...
        self.assertEqual(classify_bottleneck(result), "CPU_BOUND")
        result_no_gpu = self._make_cap_result(4, 20, gpu_util_peak_ratio=None)
        self.assertEqual(classify_bottleneck(result_no_gpu), "UNKNOWN")
    ...
    def test_hard_stop_gates_all_pass(self) -> None:
        ...
        gates = _check_hard_stop_gates(r, 0.85, 0.80, 10_000_000_000)
        self.assertTrue(_hard_stops_passed(gates))
    ...
    def test_hard_stop_ram_exceeds_threshold(self) -> None:
        ...
        gates = _check_hard_stop_gates(r, 0.85, 0.80, 10_000_000_000)
        self.assertFalse(_hard_stops_passed(gates))
    ...
    def test_safety_gate_passes_clean_result(self) -> None:
        r = self._make_cap_result(4, 20)
        self.assertTrue(_passes_safety_gates(
    ...
    def test_safety_gate_rejects_high_memory(self) -> None:
        r = self._make_cap_result(4, 20, host_memory_peak_ratio=0.90)
        self.assertFalse(_passes_safety_gates(
            r, max_error_rate=0, max_iowait=0.25,
            max_peak_memory_ratio=0.80, max_fd_util_ratio=0.80,
            min_disk_free_bytes=10_000_000_000))
    ...
    def _gate_fail_result(self, cap: int) -> CapResult:
    ...
    def test_dynamic_search_finds_boundary_via_binary(self) -> None:
        ...
        def test_fn(cap: int) -> CapResult:
            if cap <= 6:
                return self._make_cap_result(cap, 10)
            return self._gate_fail_result(cap)

        results, exp, bin_caps = dynamic_cap_search(test_fn, 10)
    ...
    def test_dynamic_search_all_pass(self) -> None:
        ...
        results, exp, bin_caps = dynamic_cap_search(test_fn, 8)
        tested = [r.max_active_jobs for r in results]
        self.assertEqual(exp, [1, 2, 4, 8])
    ...
    def test_dynamic_search_max_cap_1(self) -> None:
        ...
        results, exp, bin_caps = dynamic_cap_search(test_fn, 1)
        self.assertEqual([r.max_active_jobs for r in results], [1])
        self.assertEqual(exp, [1])
        self.assertEqual(bin_caps, [])
