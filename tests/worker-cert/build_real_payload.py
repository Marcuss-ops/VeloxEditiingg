...
def build_payload(
    ...
    `placement_pin_worker_id` optionally forces scheduler placement onto a
    specific worker; it is intentionally independent from `worker_id`, which
    only labels the generated payload. An empty value leaves placement
    unpinned.
    ...
    if placement_pin_worker_id.strip():
        payload["placement_pin_worker_id"] = placement_pin_worker_id.strip()
    return payload
...
def cmd_build(args: argparse.Namespace) -> int:
    ...
    payload = build_payload(
        ...
        placement_pin_worker_id=args.placement_pin_worker_id or "",
    )
    if args.strict:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        ...
        "    --placement-pin-worker-id host_57_131_20_173 \\\n"
    ...
    parser.add_argument(
        "--placement-pin-worker-id",
        default="",
        help="optional worker_id to force placement onto; independent from "
        "--worker-id, which only labels the payload (default: unpinned)",
    )
    parser.add_argument(
        "--no-placement-pin",
        action="store_true",
        help="deprecated compatibility alias for an empty --placement-pin-worker-id",
    )
    ...
    if args.no_placement_pin and args.placement_pin_worker_id:
        parser.error("--no-placement-pin cannot be combined with --placement-pin-worker-id")
    if args.no_placement_pin:
        args.placement_pin_worker_id = ""
