...
#      the default stub path proves the worker pipeline plumbing only
...
#   VELOX_BENCH_REAL        1 = run the REAL zero-spawn renderer over
#                           the generated track (requires the engine
#                           binary + ffmpeg/ffprobe for track generation);
#                           default 0 = stub renderer, NOT a real benchmark
#   VELOX_BENCH_ENGINE_BIN  velox_video_engine binary path (default:
#                           standard engine resolution)
...
ENGINE_BIN="${VELOX_BENCH_ENGINE_BIN:-}"
...
if [[ "$REAL" == 1 ]]; then
  ...
  printf 'Set VELOX_BENCH_REAL=1 to run the real zero-spawn renderer.\n'
fi
...
if [[ "$REAL" == 0 ]]; then
  ...
  BENCH_ARGS+=(-track-dir "$TRACK_DIR")
  [[ -n "$ENGINE_BIN" ]] && BENCH_ARGS+=(-engine-bin "$ENGINE_BIN")
fi
