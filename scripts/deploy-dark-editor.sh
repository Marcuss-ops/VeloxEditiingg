...
if [ -d "${REPO_ROOT}/VeloxFrontend/web/dark_editor" ]; then
    DARK_EDITOR_DIR="${REPO_ROOT}/VeloxFrontend/web/dark_editor"
elif [ -d "${REPO_ROOT}/../VeloxFrontend/web/dark_editor" ]; then
    # VeloxFrontend is normally checked out as a sibling repository.
    DARK_EDITOR_DIR="${REPO_ROOT}/../VeloxFrontend/web/dark_editor"
else
    echo "Dark Editor source directory not found next to ${REPO_ROOT}" >&2
    exit 1
fi
