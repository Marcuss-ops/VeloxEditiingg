...
type WorkerControlClient interface {
	...
	Stream(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[WorkerToMasterEnvelope, MasterToWorkerEnvelope], error)
}
...
func (c *workerControlClient) Stream(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[WorkerToMasterEnvelope, MasterToWorkerEnvelope], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &WorkerControl_ServiceDesc.Streams[0], WorkerControl_Stream_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[WorkerToMasterEnvelope, MasterToWorkerEnvelope]{ClientStream: stream}
	return x, nil
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type WorkerControl_StreamClient = grpc.BidiStreamingClient[WorkerToMasterEnvelope, MasterToWorkerEnvelope]
...
type WorkerControlServer interface {
	// Stream establishes a bidirectional control channel with typed messages.
	// Worker sends WorkerToMasterEnvelope, master responds with MasterToWorkerEnvelope.
	Stream(grpc.BidiStreamingServer[WorkerToMasterEnvelope, MasterToWorkerEnvelope]) error
...
func (UnimplementedWorkerControlServer) Stream(grpc.BidiStreamingServer[WorkerToMasterEnvelope, MasterToWorkerEnvelope]) error {
...
func _WorkerControl_Stream_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(WorkerControlServer).Stream(&grpc.GenericServerStream[WorkerToMasterEnvelope, MasterToWorkerEnvelope]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type WorkerControl_StreamServer = grpc.BidiStreamingServer[WorkerToMasterEnvelope, MasterToWorkerEnvelope]
