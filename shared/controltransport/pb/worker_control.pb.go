...
// ArtifactUploaded — PR 2 (chunk 4) trust boundary.
//
// DEPRECATED FIELDS (kept for wire-compat with old workers; never trusted
...
//
// All five (upload_id + lease_id + attempt + expected_revision +
// artifact_id for diagnostic correlation) must be supplied by the worker.
// A missing/empty upload_id is rejected by the handler because the master
// cannot resolve the artifact_uploads row without it.
//
// .pb.go regeneration (chunk 4+r follow-up): once the regenerated Go types
// land, handler_artifacts.go can drop its UploadID-proxy-via-artifact_id
// workaround and use the real upload_id field directly.
type ArtifactUploaded struct {
	...
	// Deprecated PR 1 fields. Kept on the wire by tag number for
	// backwards-compat with old workers but the master ignores them.
	ArtifactPath string `protobuf:"bytes,4,opt,name=artifact_path,json=artifactPath,proto3" json:"artifact_path,omitempty"`  // DEPRECATED — master recomputes from bytes
	ArtifactSize int64  `protobuf:"varint,5,opt,name=artifact_size,json=artifactSize,proto3" json:"artifact_size,omitempty"` // DEPRECATED — master recomputes from bytes
	UploadStatus string `protobuf:"bytes,6,opt,name=upload_status,json=uploadStatus,proto3" json:"upload_status,omitempty"`  // DEPRECATED — replaced by upload row
	Error        string `protobuf:"bytes,7,opt,name=error,proto3" json:"error,omitempty"`                                    // Diagnostic only, surfaced in logs
	// PR 2 required fields.
	UploadId         string `protobuf:"bytes,8,opt,name=upload_id,json=uploadId,proto3" json:"upload_id,omitempty"`                           // master-side session id (BeginUpload)
	LeaseId          string `protobuf:"bytes,9,opt,name=lease_id,json=leaseId,proto3" json:"lease_id,omitempty"`                              // CAS-gate for finalization
	Attempt          int32  `protobuf:"varint,10,opt,name=attempt,proto3" json:"attempt,omitempty"`                                           // attempt_number
	ExpectedRevision int32  `protobuf:"varint,11,opt,name=expected_revision,json=expectedRevision,proto3" json:"expected_revision,omitempty"` // job.revision at lease acceptance
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}
...
const file_proto_velox_control_worker_control_proto_rawDesc = "" +
	...
	"\x11expected_revision\x18\v \x01(\x05R\x10expectedRevision\"\xea\x06\n" +
