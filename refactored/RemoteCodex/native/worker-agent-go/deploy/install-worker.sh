...
#   --fallback         Enable Python fallback for rollback (default: true)
#   --no-fallback      Disable Python fallback
...
echo -e "Python Fallback: ${GREEN}$ENABLE_FALLBACK${NC}"
...
# Create Python fallback directory if enabled
if $ENABLE_FALLBACK; then
    run_cmd mkdir -p "$WORK_DIR/ServerRemote"
fi
...
# Step 6: Setup Python fallback (if enabled)
if $ENABLE_FALLBACK; then
    echo ""
    echo -e "${BLUE}Step 6: Setting up Python fallback...${NC}"
    
    # Copy ServerRemote if it exists in project
    SERVER_REMOTE_SRC="$SCRIPT_DIR/../../ServerRemote"
    if [[ -d "$SERVER_REMOTE_SRC" ]]; then
        if ! $DRY_RUN; then
            cp -r "$SERVER_REMOTE_SRC"/* "$WORK_DIR/ServerRemote/" 2>/dev/null || true
            echo -e "${GREEN}Copied ServerRemote to $WORK_DIR/ServerRemote/${NC}"
        else
            echo -e "${YELLOW}[DRY-RUN] Would copy ServerRemote${NC}"
        fi
    else
        echo -e "${YELLOW}ServerRemote source not found at $SERVER_REMOTE_SRC${NC}"
    fi
    
    # Create venv if needed
    if [[ ! -d "$WORK_DIR/venv" ]]; then
        run_cmd python3 -m venv "$WORK_DIR/venv"
    fi
fi
...
# ExecStart=$WORK_DIR/venv/bin/python3 $WORK_DIR/ServerRemote/job_worker.py \\
#     --master $MASTER_URL \\
#     --name $WORKER_ID

# Environment
Environment=VELOX_WORKER_MODE=go
Environment=VELOX_FALLBACK_ENABLED=$ENABLE_FALLBACK
