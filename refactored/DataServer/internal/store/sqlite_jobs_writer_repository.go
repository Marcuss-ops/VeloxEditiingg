...
//
// These primitives exist alongside the legacy methods above so callers
...
// ArtifactSuccessGate created in bootstrap may call this method; the
// public LifecycleService never exposes SUCCEEDED.
//
// Single-tx shape: UPDATE jobs → SUCCEEDED + completed_at, INSERT history,
// INSERT event, INSERT outbox (JOB_SUCCEEDED).
func (r *SQLiteJobRepository) PR3MarkSucceeded(ctx context.Context, cmd MarkSucceededCommand) error {
	if cmd.JobID == "" {
		return fmt.Errorf("PR3MarkSucceeded: empty jobID")
	}

	tx, err := r.store.db.BeginTx(ctx, nil)
	if err != nil {
		return fmt.Errorf("PR3MarkSucceeded begin: %w", err)
	}
	defer func() { _ = tx.Rollback() }()

	// Idempotent on already-SUCCEEDED.
	var current string
	if err := tx.QueryRowContext(ctx, `SELECT UPPER(COALESCE(status,'')) FROM jobs WHERE job_id = ?`, cmd.JobID).Scan(&current); err != nil {
		if err == sql.ErrNoRows {
			return fmt.Errorf("PR3MarkSucceeded %s: not found", cmd.JobID)
		}
		return fmt.Errorf("PR3MarkSucceeded status: %w", err)
	}
	...
	res, err := tx.ExecContext(ctx,
		`UPDATE jobs
		   SET status = 'SUCCEEDED',
		       ...
		       assigned_at = ''
		 WHERE job_id = ?
		   AND UPPER(COALESCE(status, '')) IN ('RENDER_FINISHED', 'AWAITING_ARTIFACT', 'RUNNING', 'LEASED', 'RETRY_WAIT')
		   AND revision = ?`,
		now, now, cmd.JobID, cmd.ExpectedRevision,
	)
	if err != nil {
		return fmt.Errorf("PR3MarkSucceeded UPDATE: %w", err)
	}
	if n, _ := res.RowsAffected(); n == 0 {
		return fmt.Errorf("PR3MarkSucceeded %s: %w", cmd.JobID, ErrTransitionConflict)
	}

	if err := r.insertHistoryTx(ctx, tx, cmd.JobID, "SUCCEEDED", cmd.WorkerID, "Artifact verified and registered"); err != nil {
		return fmt.Errorf("PR3MarkSucceeded history: %w", err)
	}
	if err := r.insertEventTx(ctx, tx, cmd.JobID, "job_succeeded", map[string]interface{}{
		"worker_id":    cmd.WorkerID,
		"artifact_id":  cmd.ArtifactID,
		"attempt_id":   cmd.AttempterID,
	}); err != nil {
		return fmt.Errorf("PR3MarkSucceeded event: %w", err)
	}
	...
	if err := r.store.emitOutbox(ctx, tx, outbox.InsertParams{
		...
		return fmt.Errorf("PR3MarkSucceeded outbox: %w", err)
	}

	if err := tx.Commit(); err != nil {
		return fmt.Errorf("PR3MarkSucceeded commit: %w", err)
	}
	return nil
}
