...
// DriveOAuthStartHandlerFunc exposes the same OAuth start flow without needing a DriveHandlers instance.
func DriveOAuthStartHandlerFunc(driveService *integrationsDrive.Service) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		authURL := integrationsDrive.GetAuthURL(driveService.GetOAuthConfig(), state)
		c.JSON(http.StatusOK, gin.H{
			"ok":         true,
			"auth_url":   authURL,
			"token_name": tokenName,
...
// DriveOAuthCallbackHandlerFunc stores the Drive token returned by Google without needing a DriveHandlers instance.
func DriveOAuthCallbackHandlerFunc(driveService *integrationsDrive.Service) gin.HandlerFunc {
	return func(c *gin.Context) {
		if driveService == nil {
			c.JSON(http.StatusServiceUnavailable, gin.H{"ok": false, "error": "drive service not configured"})
			return
		...
		token, err := integrationsDrive.ExchangeCode(ctx, driveService.GetOAuthConfig(), code)
		if err != nil {
			log.Printf("[DRIVE] OAuth callback failed: %v", err)
			c.JSON(http.StatusInternalServerError, gin.H{"ok": false, "error": err.Error()})
			return
		...
		if err := driveService.GetTokenManager().SaveToken(state, token); err != nil {
			log.Printf("[DRIVE] Failed to persist token %s: %v", state, err)
			c.JSON(http.StatusInternalServerError, gin.H{"ok": false, "error": err.Error()})
			return
		}
