...
| Telemetry catalog | one registry | single language-neutral source `shared/telemetry/catalog.json` (loaded by `catalog_source.go`, C++ binding regenerated via `cmd/cataloggen`, freshness pinned by `scripts/ci/check-telemetry-catalog.sh`); no parallel `canonicalEventKeys`/`canonicalOriginScope`/`canonicalPhaseSpecs`; every entry resolves the full `TelemetryDescriptor` (`Kind`/`Unit`/`TimingMode`/`Aggregation`/`Cardinality`/`Owner`) at declaration time — receipt-level facts map to exactly one owner via the JSON `facts` table / `fact_owner.go`, no ad-hoc owner assignments; `accounted_ratio` sums ONLY `timing_mode=exclusive` (per_attempt) durations, never span children |
