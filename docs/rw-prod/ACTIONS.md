...
|----|--------------|--------|-------|-----------|-------|----------|--------|---------------------------|
| LEGACY-A1 | `DataServer/internal/socialclient/targets.go` `ListPublishingTargets`; test in `targets_test.go` | Migrare i test a `ListPublishingCatalog` e rimuovere il wrapper non usato in produzione | CHIUSO | RIMOSSO | social-platform | P1 | Medio: possibile riferimento test o consumer interno non rilevato | `git grep -n 'ListPublishingTargets' -- '*.go'` vuoto; `go test ./internal/socialclient/...`; `scripts/ci/pre-removal-verify.sh` verde |
| LEGACY-A2 | `DataServer/internal/socialclient/targets.go` `PublishingTargetCatalogRequest` | Sostituire l’alias con `PublishingCatalogRequest` e rimuovere il tipo compatibile | CHIUSO | RIMOSSO | social-platform | P1 | Basso: alias interno senza comportamento proprio | `git grep -n 'PublishingTargetCatalogRequest' -- '*.go'` vuoto; wire shape invariata; test socialclient, build e `scripts/ci/pre-removal-verify.sh` verdi |
