...
| 14 | Metriche / alert | `internal/telemetry/metrics_types.go:151` espone `velox_fallback_count_total`, `velox_python_emergency_path_total`. `alerts/spec-14-compute-outcomes.yml` — alert su compute-outcomes. `docs/operations/PR-6-pki-rotation-runbook.md:179` — `alert-cert-expiry.sh` **deliberatamente NON versionato** (vedi RW-PROD-013). | ⚠️ non tutte le soglie RW-PROD-013 hanno alert; correlazione `worker_id + job_id + task_id + attempt_id + session_id` da verificare. |
