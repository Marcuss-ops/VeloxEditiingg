...
## YouTube→Social closure mirror (historical cross-reference)

This companion changelog no longer duplicates the historical migration record. The canonical closure record, including its committed migration history and operator-facing retirement details, is maintained in the root [`CHANGELOG.md`](../CHANGELOG.md). The Social API migration runbook remains the operational source for current fail-closed behavior.
