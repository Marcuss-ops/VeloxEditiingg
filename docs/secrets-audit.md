...
> **Scopo:** inventario completo dei secret presenti nell'ecosistema Velox
> (repo, CI, master, worker, workstation operatore) come **baseline di verifica**
> della migrazione a OpenBao.
> **Metodo:** scansione di template committati (`*.example`), bridge Ansible
> mantenuti per compatibilità, workflow GitHub Actions, script shell, codice Go
> e documentazione.
...
|---|---|---|
| **OpenBao KV/AppRole** | `deploy/openbao/` e `velox/production/...` | token admin, JWT secret, social token, worker credentials, policy |
| **File env su host** | `/etc/velox-server.env`, `/etc/velox-worker/worker.env`, `/etc/velox-worker-<host>.env` | token, JWT secret, `VELOX_WORKER_SECRET` materializzati dal resolver |
| **File secret su disco** | `/etc/velox-worker/secrets/worker_credential`, `/opt/velox/secrets/admin-token`, `DataServer/data/drive/credentials/` | credential worker legacy, admin token, OAuth Drive |
| **PKI / certificati** | OpenBao PKI (`initialize-pki-intermediate.sh` + `provision-pki.sh`), listener TLS `gen-tls.sh`, rinnovo `velox-worker-mtls-renew.timer` | chiavi private locali + certificati mTLS |
...
### 2.1 Ansible Vault — storico/compatibility bridge

I nomi `vault_velox_*` e la struttura `deploy/group_vars/vault.yml` sono
legacy. Il resolver Master canonical usa OpenBao KV/AppRole e materializza
le chiavi direttamente nell'env runtime; non reintrodurre YAML Ansible per il
deploy production. Le righe sotto restano come inventario storico dei nomi e
dei consumatori del bridge ancora mantenuto per la migrazione di host legacy.
**Ogni referente Ansible/Jinja elencato nella tabella è quindi un bridge legacy,
non un consumer del rollout o della materializzazione production canonical.**

| Variabile | Env renderizzato | Referente (consumatore) | Tipo |
|---|---|---|---|
| `vault_velox_admin_token` | `VELOX_ADMIN_TOKEN` (via legacy `deploy/templates/velox-server.env.j2`) | Master REST `AdminAuthMiddleware`; legacy/bridge playbook `fleet-*` (`fleet-status/rollback/update/drain/health/smoke`) come `Authorization: Bearer` | Token statico (admin) |
...
|---|---|---|---|
| `VELOX_ADMIN_TOKEN` | AdminAuthMiddleware REST | Token statico | materializzato da OpenBao resolver; placeholder solo nei template |
| `INSTAEDIT_CONTROL_JWT_SECRET` | verifier InstaEdit (HS256) | Secret condiviso | materializzato da OpenBao; ≥32 byte; accoppiato con InstaEdit |
| `SOCIAL_API_TOKEN` | socialclient | Token statico | materializzato da OpenBao resolver |
| `SOCIAL_WEBHOOK_SECRET` | (futuro) HMAC callback | Secret HMAC | opzionale OpenBao KV |
| `VELOX_COMMIT_HMAC_KEY` | Completion coordinator / firma HMAC artifact (`DataServer/internal/config/config.go:151`, `bootstrap_assets.go`) | Chiave HMAC (hex ≥32 byte) | **Obbligatoria in production** (`config.go:156`); materializzare da OpenBao in `/etc/velox-server.env` |
...
**Due flussi di credential worker coesistono durante la migrazione:**
...
2. *Env-based (canonical):* `VELOX_WORKER_SECRET` in `/etc/velox-worker/worker.env`,
   scritto dal runtime canonical; il vecchio `canonical_worker_runtime.yml` resta
   solo bridge legacy. Il gate pre-restart blocca il deploy se il master ha già
   una credential salvata per il worker e l'env non la porta.
...
|---|---|---|---|
| Chiave privata operatore | `~/.ssh/velox` (o `~/.ssh/id_ed25519_velox`, vedi fixture test) | legacy `bootstrap-ssh.yml`, inventory `ansible_ssh_private_key_file` | Chiave privata SSH |
...
| **CA SSH OpenBao (pubblica)** | `.velox/openbao/ssh-ca.pub` → `/etc/ssh/trusted-user-ca-keys.pem` via legacy `bootstrap-ssh.yml` (`vault_velox_ssh_ca_pubkey`), `TrustedUserCAKeys` | tutti i nodi (verifica cert) | Chiave pubblica CA |
...
| SecretResolver SSH | `DataServer/internal/handlers/remote/ansible/secrets.go` — file `ssh_host_<host>` (0600) sotto il secrets dir; la tabella `ansible_hosts` (SQLite) salva solo `secret_ref` (`file:ssh_host_<host>`), mai la password in chiaro | compatibility-bridge inventory dinamico Ansible (`manager_computers.go`) | Password SSH (transitorie) |
...
| Root CA cert | dispositivo/artefatto pubblico della cerimonia OpenBao | PEM | pubblico |
| Intermediate CA key | interna a OpenBao PKI | chiave non esportata | online |
| OpenBao listener cert | `.velox/openbao/tls/server.crt` + `.key` | PEM | TLS del secrets manager |
| Worker leaf cert | `/etc/velox-worker/certs/current/` → CSR locale + OpenBao PKI | PEM | leaf mTLS, CN=worker_id |
...
| Rotazione | `velox-worker-mtls-renew.timer` + resolver OpenBao `--renew`/`--check` | systemd + OpenBao PKI | Rinnovo locale sul worker; nessuna chiave privata o cert worker sul Master |
...
|---|---|---|
| `secrets.ANSIBLE_VAULT_PASSWORD` | eventuale workflow/bridge legacy | solo compatibilità; non è fonte dei secret production canonical |
...
| Elemento | Dove | Note |
|---|---|---|
| `.velox/production.env` | `.velox/` (gitignored, `chmod 600`) | `VELOX_MASTER_URL`, `VELOX_ADMIN_TOKEN`, GHCR repo; eventuale `ANSIBLE_VAULT_PASSWORD` è solo bridge legacy |
...
|---|---|---|---|---|---|---|
| 1 | `VELOX_ADMIN_TOKEN` | OpenBao KV/AppRole → `/etc/velox-server.env`; `/opt/velox/secrets/admin-token`; CI secret | token | master REST, fleetctl, operator | token statico | **alta** (god-mode REST) |
| 2 | `INSTAEDIT_CONTROL_JWT_SECRET` | OpenBao KV → `/etc/velox-server.env` | HS256 ≥32B | verifier InstaEdit (accoppiato BFF) | secret condiviso | alta |
| 3 | `SOCIAL_API_TOKEN` | OpenBao KV → `/etc/velox-server.env` | bearer | socialclient | token statico | alta |
...
1. **OpenBao è la fonte canonical** dei secret production statici (token, JWT,
   worker credentials e PKI). Ansible Vault e `vault_velox_*` restano soltanto
   riferimenti storici o bridge di migrazione e non devono alimentare il deploy
...
5. **Gap individuati:**
   ...
   - Il rinnovo PKI worker è già affidato a `velox-worker-mtls-renew.timer` e al
     resolver OpenBao; non reintrodurre cron o script di rotazione basati su PEM
     locali sul Master.
