...
Cross-client authorisation is **STRICT**: every M2M polling read and action resolves the requested job through `creator_forwardings` and requires `external_client_id == m2m_client_id`. This covers `GET /api/v1/jobs/:id`, `GET /api/v1/jobs/:id/asset-progress`, and every `/api/v1/pipeline-runs/:id` operation (status, cancel, retry, timeline, artifacts, deliveries). A job that does not exist AND a job owned by another client return the identical `404 job_not_found` envelope (`ok:false`, `error:"job_not_found"`, same `message`) — the boundary reveals no existence oracle. Artifact URLs, delivery rows, timeline events and asset-progress are surfaced only after the ownership-scoped lookup succeeds; the same rule applies to the artifact / delivery projections nested in the status response. Admin callers (operator `VELOX_ADMIN_TOKEN`) bypass the M2M boundary by design and keep the unscoped admin surface.
