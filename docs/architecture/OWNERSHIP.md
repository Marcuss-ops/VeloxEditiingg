...
| Per-job `costmodel.JobRequirements` threading (PR-04.5) | `velox-server/internal/costmodel.JobRequirements` is the single source of truth. Read path: `jobs.Writer.Get → jobs.Job.Requirements → jobs.QueueItem.Requirements`. Write path: `Enqueuer.Enqueue → JobQueue.SubmitJob → jobs.Writer.Create` (mirrors into both dedicated columns `job_required_resource_class` / `job_required_temporal_mode` AND `_requirements` sub-object inside `request_json`). | Hand-decoded Requirements at dispatch sites (`grpcserver`, `workers`, `outbox`); second `JobRequirements` type aliases; bypass of the canonical `costmodel.JobRequirements` shape; in-place re-parsing of `request_json` to recover requirements when `jobs.Job.Requirements` is already populated. The default `costmodel.DefaultRequirements()` is the only permissive fallback and MUST stay so. |
| Legacy Workflow v2 state (DECOMMISSIONING — PR operations 01) | Read-only: `velox-server/internal/workflow` (Repository, SQLiteRepository, types, migrate). Status: **DECOMMISSIONING**. Cutover sequence: docs/operations/01-workflow-taskgraph-cutover.md specifies the 8-phase plan; the canonical writer is the combination of `taskgraph`, `taskattempts`, `jobs`, `renderjobs.CreateService` (PR-operation 01 Fase 2) and `artifacts.Service`. | NEW imports of `velox-server/internal/workflow` (forbidden in PR-scoped `check-no-legacy.sh`); NEW call sites of `CreateRun`, `MarkStepRunning`, `CompleteStepAndReleaseDependents`, `FailStep`, `CancelRun` (scanned in `check-single-writer.sh`); SQL writes to `workflow_runs` / `workflow_steps` / `workflow_dependencies` / `workflow_events` outside the already-censured legacy files; new `WORKFLOW_*` outbox event types. Removal deadline is set in the COMPATIBILITY shim header of each file. |
