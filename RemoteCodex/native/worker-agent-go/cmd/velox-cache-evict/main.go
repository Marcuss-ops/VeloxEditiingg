...
func main() {
	...
	indexPath := flags.String("index", "", "path to the worker cache SQLite index (required with --execute)")
	...
	flags.Usage = func() {
		fmt.Fprintf(flags.Output(), "Usage: %s --cache-root ROOT --asset-id ID [--asset-id ID ...] [--index PATH] [--execute]\n\n", os.Args[0])
		fmt.Fprintln(flags.Output(), "Plans exact asset-cache file removals by default. --execute requires an existing SQLite index so active leases can be refused.")
		flags.PrintDefaults()
	}
	...
	if !*execute && strings.TrimSpace(*indexPath) != "" {
		fmt.Fprintln(os.Stderr, "error: --index is only supported with --execute; dry-run must not open or mutate SQLite")
		os.Exit(2)
	}
	if strings.TrimSpace(*indexPath) != "" {
		...
		fmt.Fprintln(os.Stderr, "error: --index is required with --execute")
		os.Exit(2)
	}

	var index *workercache.Cache
	if strings.TrimSpace(*indexPath) != "" {
		opened, err := workercache.Open(*indexPath)
		if err != nil {
