...
func main() {
	...
	if err := ensureWorkerConsole(currentDir); err != nil {
		log.Fatalf("failed to prepare worker-console scaffold: %v", err)
	}
...
func ensureWorkerConsole(currentDir string) error {
	base := filepath.Join(currentDir, "RemoteCodex", "worker-console")
	distDir := filepath.Join(base, "web", "dist")
	if err := os.MkdirAll(distDir, 0755); err != nil {
		return err
	}

	dockerfile := filepath.Join(base, "Dockerfile.console")
	...
	if _, err := os.Stat(index); os.IsNotExist(err) {
		html := "<!doctype html><html><head><meta charset=\"utf-8\"><title>Velox Worker Console</title></head><body><h1>Velox Worker Console</h1><p>Bootstrap scaffold generated by velox-installer.</p></body></html>\n"
		if err := os.WriteFile(index, []byte(html), 0644); err != nil {
			return err
		}
	}
