		...
		for {
			...
			if len(line) > 0 {
				...
				var prog struct {
					...
					SafeOffsetBytes    int64   `json:"safe_offset_bytes"`
				...
				if json.Unmarshal([]byte(line), &prog) == nil {
					if prog.Event == "artifact_write_progress" {
						if fn := pipeline.ArtifactWriteCallback(ctx); fn != nil {
							fn(pipeline.ArtifactWriteProgress{
								Artifact:           prog.Artifact,
								Path:               prog.Path,
								HighWatermarkBytes: prog.HighWatermarkBytes,
								SafeOffsetBytes:    prog.SafeOffsetBytes,
