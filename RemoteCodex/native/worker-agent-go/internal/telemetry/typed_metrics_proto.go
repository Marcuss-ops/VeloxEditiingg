...
func (t RawExecutionMetrics) ToProto() *pb.TaskExecutionMetrics {
	return &pb.TaskExecutionMetrics{
		...
		PacketCopyRatio:                     t.PacketCopyRatio,
		...
		JobsConcurrentAtStart:               t.JobsConcurrentAtStart,
		...
		JobPeakRssDeltaBytes:                t.JobPeakRssDeltaBytes,
		JobCpuCoreSeconds:                   t.JobCpuCoreSeconds,
		JobAssetCacheBytesUsed:              t.JobAssetCacheBytesUsed,
		JobPrefetchBytes:                    t.JobPrefetchBytes,
		JobUploadBufferPeakBytes:            t.JobUploadBufferPeakBytes,
		JobRenderWallMs:                     t.JobRenderWallMs,
		JobAssetWallMs:                      t.JobAssetWallMs,
		JobPublishWallMs:                    t.JobPublishWallMs,
		ProgressiveOverlapFirstPartMs:       t.ProgressiveOverlapFirstPartMs,
		ProgressiveOverlapPartsBeforeRender: t.ProgressiveOverlapPartsBeforeRender,
		ProgressiveOverlapBytesBeforeRender: t.ProgressiveOverlapBytesBeforeRender,
		ProgressiveOverlapMs:                t.ProgressiveOverlapMs,
		TrailerToOpenMs:                     t.TrailerToOpenMs,
		MuxToOpenUs:                         t.MuxToOpenUS,
		SceneCount:                          t.SceneCount,
		ThroughputX:                         t.ThroughputX,
		CriticalPathPercent:                 t.CriticalPathPercent,
		CriticalPathMs:                      t.CriticalPathMs,
		CriticalPathComponent:               t.CriticalPathComponent,
		GpuUtilAvgPercent:                   t.GpuUtilAvgPct,
		GpuUtilPeakPercent:                  t.GpuUtilPeakPct,
		NvdecUtilAvgPercent:                 t.NvdecUtilAvgPct,
		NvdecUtilPeakPercent:                t.NvdecUtilPeakPct,
		NvencUtilAvgPercent:                 t.NvencUtilAvgPct,
		NvencUtilPeakPercent:                t.NvencUtilPeakPct,
		VramUsedAvgBytes:                    t.VramUsedAvgBytes,
		GpuIdleDuringRenderMs:               t.GpuIdleMs,
		FramesDownloadedFromGpu:             t.FramesDownloadedFromGPU,
		FramesUploadedToGpu:                 t.FramesUploadedToGPU,
		GpuToCpuBytes:                       t.GpuToCpuBytes,
		CpuToGpuBytes:                       t.CpuToGpuBytes,
		SegmentsTotal:                       t.SegmentsTotal,
		SegmentsPacketCopy:                  t.SegmentsPacketCopy,
		SegmentsReencoded:                   t.SegmentsReencoded,
		PacketCopyBytes:                     t.PacketCopyBytes,
		ReencodedBytes:                      t.ReencodedBytes,
	}
}
...
func FromProto(p *pb.TaskExecutionMetrics) RawExecutionMetrics {
	...
	return RawExecutionMetrics{
		...
		PacketCopyRatio:                     p.GetPacketCopyRatio(),
		...
		JobsConcurrentAtStart:               p.GetJobsConcurrentAtStart(),
		...
		JobPeakRssDeltaBytes:                p.GetJobPeakRssDeltaBytes(),
		JobCpuCoreSeconds:                   p.GetJobCpuCoreSeconds(),
		JobAssetCacheBytesUsed:              p.GetJobAssetCacheBytesUsed(),
		JobPrefetchBytes:                    p.GetJobPrefetchBytes(),
		JobUploadBufferPeakBytes:            p.GetJobUploadBufferPeakBytes(),
		JobRenderWallMs:                     p.GetJobRenderWallMs(),
		JobAssetWallMs:                      p.GetJobAssetWallMs(),
		JobPublishWallMs:                    p.GetJobPublishWallMs(),
		ProgressiveOverlapFirstPartMs:       p.GetProgressiveOverlapFirstPartMs(),
		ProgressiveOverlapPartsBeforeRender: p.GetProgressiveOverlapPartsBeforeRender(),
		ProgressiveOverlapBytesBeforeRender: p.GetProgressiveOverlapBytesBeforeRender(),
		ProgressiveOverlapMs:                p.GetProgressiveOverlapMs(),
		TrailerToOpenMs:                     p.GetTrailerToOpenMs(),
		MuxToOpenUS:                         p.GetMuxToOpenUs(),
		SceneCount:                          p.GetSceneCount(),
		ThroughputX:                         p.GetThroughputX(),
		CriticalPathPercent:                 p.GetCriticalPathPercent(),
		CriticalPathMs:                      p.GetCriticalPathMs(),
		CriticalPathComponent:               p.GetCriticalPathComponent(),
		GpuUtilAvgPct:                       p.GetGpuUtilAvgPercent(),
		GpuUtilPeakPct:                      p.GetGpuUtilPeakPercent(),
		NvdecUtilAvgPct:                     p.GetNvdecUtilAvgPercent(),
		NvdecUtilPeakPct:                    p.GetNvdecUtilPeakPercent(),
		NvencUtilAvgPct:                     p.GetNvencUtilAvgPercent(),
		NvencUtilPeakPct:                    p.GetNvencUtilPeakPercent(),
		VramUsedAvgBytes:                    p.GetVramUsedAvgBytes(),
		GpuIdleMs:                           p.GetGpuIdleDuringRenderMs(),
		FramesDownloadedFromGPU:             p.GetFramesDownloadedFromGpu(),
		FramesUploadedToGPU:                 p.GetFramesUploadedToGpu(),
		GpuToCpuBytes:                       p.GetGpuToCpuBytes(),
		CpuToGpuBytes:                       p.GetCpuToGpuBytes(),
		SegmentsTotal:                       p.GetSegmentsTotal(),
		SegmentsPacketCopy:                  p.GetSegmentsPacketCopy(),
		SegmentsReencoded:                   p.GetSegmentsReencoded(),
		PacketCopyBytes:                     p.GetPacketCopyBytes(),
		ReencodedBytes:                      p.GetReencodedBytes(),
	}
}
