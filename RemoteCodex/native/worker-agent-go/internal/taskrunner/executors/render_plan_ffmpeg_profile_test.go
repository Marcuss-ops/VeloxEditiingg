...
// sinkExecutionContext is a minimal ExecutionContext stub. The remaining
// interface methods are noops; executor tests only need a valid context.
...
func TestEncodeExecutor_ConsumesCanonicalRunnerAndPublishesTypedMetrics(t *testing.T) {
	...
	if result.RawMetrics == nil || result.RawMetrics.FfmpegProcessMs != 340 {
		t.Errorf("typed ffmpeg process metrics = %+v, want process wall 340", result.RawMetrics)
	}
...
func TestComposeExecutor_RunnerReceivesComposeOperation(t *testing.T) {
	...
	if result.RawMetrics == nil {
		t.Error("raw ffmpeg metrics missing from compose result")
	}
}
