...
//
// The typed artifact publication (publishArtifactsV1 /
// registerOutputSpool / rejectOutputSpool) lives in the sibling file
// active_task_publish.go.
//
...
func (w *Worker) uploadTaskOutputs(ctx context.Context, pte *PendingTaskExecution, report *taskrunner.TaskExecutionReport) (err error) {
	...
	// every declared output (the video and its progress receipt) in one
	// attempt-level declaration/commit, so TaskCommitAck cannot acknowledge
	// a partial artifact set. Hand-built legacy test workers leave this nil
	// and retain the established video upload behavior below.
	if w.transport != nil && w.publisherRegistry != nil {
		return w.publishArtifactsV1(ctx, pte, report)
	}
	if w.apiClient == nil {
